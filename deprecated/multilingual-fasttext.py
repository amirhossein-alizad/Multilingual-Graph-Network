...
data = english_sentences + german_sentences

print("Creating multilingual model")
multilingual_model = FastText(sentences=data, vector_size=100, min_count=1)
        ...
        for entity in entities:
            ...
            for other_entity in entities:
                if entity != other_entity:
                    ...
                    similarity = calculate_similarity(entity.split('_')[0], other_entity.split('_')[0], model)
...
english_graph = build_graph(english_data_preprocessed, multilingual_model, 'english')
print(len(english_graph.nodes))
print(english_graph.number_of_edges())

print("Creating german graph")
german_graph = build_graph(german_data_preprocessed, multilingual_model, 'german')
    ...
    for german_node, german_data in german_graph.nodes(data=True):
        ...
        if len(english_node_parts) == 2 and len(german_node_parts) == 2:
            ...
            if english_node_type == german_node_type:
                similarity = calculate_similarity(eng_text, ger_text, multilingual_model)
            ...
            similarity = calculate_similarity(english_node, german_node, multilingual_model)
